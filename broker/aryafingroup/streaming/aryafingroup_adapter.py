...
from broker.aryafingroup.streaming.aryafingroup_websocket import AryafingroupWebSocketClient
...
from .aryafingroup_mapping import AryafingroupExchangeMapper, AryafingroupCapabilityRegistry
from database.token_db import get_symbol


class AryafingroupWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """aryafingroup XTS specific implementation of the WebSocket adapter"""
    
    def __init__(self):
        super().__init__()
        self.logger = logging.getLogger("aryafingroup_websocket")
        self.ws_client = None
        self.user_id = None
        self.broker_name = "aryafingroup"
        ...
        self.logger.info(f"aryafingroup adapter initialized with ZMQ port: {self.zmq_port}")
    
    def initialize(self, broker_name: str, user_id: str, auth_data: Optional[Dict[str, str]] = None) -> None:
        """
        Initialize connection with aryafingroup XTS WebSocket API
        
        Args:
            broker_name: Name of the broker (always 'aryafingroup' in this case)
        ...
        if not auth_data:
            ...
            if not api_key or not api_secret:
                ...
                raise ValueError("Missing aryafingroup XTS API credentials in environment variables")
        ...
        self.logger.info(f"Using API Key: {api_key[:10]}... for aryafingroup XTS connection")
        
        # Create aryafingroup WebSocket client with API credentials
        self.ws_client = AryafingroupWebSocketClient(
            api_key=api_key,
    ...
    def connect(self) -> None:
        """Establish connection to aryafingroup XTS WebSocket"""
    ...
    def _connect_with_retry(self) -> None:
        """Connect to aryafingroup XTS WebSocket with retry logic"""
        while self.running and self.reconnect_attempts < self.max_reconnect_attempts:
            try:
                self.logger.info(f"Connecting to aryafingroup XTS WebSocket (attempt {self.reconnect_attempts + 1})")
    ...
    def disconnect(self) -> None:
        """Disconnect from aryafingroup XTS WebSocket"""
        self.logger.info("*** DISCONNECT CALLED - Starting aryafingroup disconnect process ***")
    ...
    def cleanup_zmq(self) -> None:
        ...
        try:
            ...
            self.logger.info("aryafingroup WebSocket cleanup completed successfully")
        except Exception as e:
            self.logger.exception(f"Error cleaning up ZeroMQ resources: {e}")
    
    def subscribe(self, symbol: str, exchange: str, mode: int = 2, depth_level: int = 5) -> Dict[str, Any]:
        """
        Subscribe to market data with aryafingroup XTS specific implementation
        ...
        if mode == 3:  # Depth mode
            if not AryafingroupCapabilityRegistry.is_depth_level_supported(exchange, depth_level):
                # If requested depth is not supported, use the highest available
                actual_depth = AryafingroupCapabilityRegistry.get_fallback_depth_level(
        ...
        # Create instrument list for aryafingroup XTS API
        exchange_type = AryafingroupExchangeMapper.get_exchange_type(brexchange)
    ...
    def _get_exchange_segment(self, exchange: str) -> str:
        ...
        """
        return AryafingroupExchangeMapper.get_exchange_type(exchange)
    ...
    def unsubscribe(self, symbol: str, exchange: str, mode: int = 2) -> Dict[str, Any]:
        ...
        # Create instrument list for aryafingroup XTS API
        instruments = [{
            "exchangeSegment": AryafingroupExchangeMapper.get_exchange_type(brexchange),
    ...
    def _on_open(self, wsapp) -> None:
        ...
        self.logger.info("Connected to aryafingroup XTS WebSocket")
        self.connected = True
    ...
    def _on_error(self, wsapp, error) -> None:
        ...
        self.logger.error(f"aryafingroup XTS WebSocket error: {error}")
    
    def _on_close(self, wsapp) -> None:
        """Callback when connection is closed"""
        self.logger.info("aryafingroup XTS WebSocket connection closed")
        self.connected = False
    ...
    def _on_data(self, wsapp, message) -> None:
        ...
        try:
            self.logger.info(f"RAW aryafingroup DATA: Type: {type(message)}, Data: {message}")
    ...
    def _process_json_data(self, data: dict):
        ...
        try:
            ...
            # Based on aryafingroup API documentation:
    ...
    def _normalize_market_data(self, message: Dict[str, Any], mode: int) -> Dict[str, Any]:
        ...
        if message_code == 1502 and 'Touchline' in message:
            ...
            # Note: 'LastTradedQunatity' is intentionally misspelled to match broker API response
