...
class AryafingroupExchangeMapper:
    """Maps between OpenAlgo exchange codes and Aryafingroup XTS specific exchange types"""
    
    # Exchange type mapping for Aryafingroup XTS broker
    # Format: {OpenAlgo_Exchange: aryafingroup_Exchange_Code}
    ...
    # Reverse mapping for converting Aryafingroup exchange codes to OpenAlgo format
    # Format: {aryafingroup_Exchange_Code: OpenAlgo_Exchange}
    ...
    def get_exchange_type(exchange):
        """
        Convert OpenAlgo exchange code to aryafingroup XTS specific exchange type
        
        Args:
            exchange: Exchange code (e.g., 'NSE', 'BSE', 'NSEFO')
            
        Returns:
            int: Exchange type code for aryafingroup XTS API
        """
        if exchange is None:
        ...
        # Mapping based on aryafingroup API documentation:
        # "NSECM": 1, "NSEFO": 2, "NSECD": 3, "BSECM": 11, "BSEFO": 12, "MCXFO": 51
    ...
    @staticmethod
    def get_openalgo_exchange(aryafingroup_code):
        """
        Convert Aryafingroup XTS exchange code to OpenAlgo exchange code
        
        Args:
            aryafingroup_code (int): aryafingroup exchange code

        Returns:
            str: OpenAlgo exchange code
        """
        return AryafingroupExchangeMapper.REVERSE_EXCHANGE_TYPES.get(aryafingroup_code, 'NSE')  # Default to NSE if not found


class AryafingroupCapabilityRegistry:
    """
    Registry of Aryafingroup XTS broker's capabilities including supported exchanges, 
    subscription modes, and market depth levels
    """
    
    # Aryafingroup XTS broker capabilities
