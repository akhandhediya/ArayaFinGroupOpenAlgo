...
from broker.aryafingroup.database.master_contract_db import SymToken, db_session
...
def get_api_response(endpoint, auth, method="GET", payload='', feed_token=None, params=None):
    ...
    base_url = URLConfig.MARKET_DATA_URL  # Default to market data URL
...
class BrokerData:
    def __init__(self, auth_token, feed_token=None, user_id=None):
        """Initialize aryafingroup data handler with authentication token"""
        self.auth_token = auth_token
        self.feed_token = feed_token
        self.user_id = user_id
        
        # Map common timeframe format to aryafingroup intervals
        self.timeframe_map = {
    ...
    def _fetch_market_data(self, token: dict, message_code: int) -> dict:
        """
        Helper method to fetch market data from aryafingroup API
        Args:
    ...
    def get_history(self, symbol, exchange, timeframe, from_date, to_date):
        ...
        try:
            ...
            while current_start <= to_date:
                ...
                # aryafingroup expects MMM DD YYYY HHMMSS in IST
                ...
                if not response or response.get('type') != 'success':
                    ...
                    raise Exception(f"Error from aryafingroup API: {response.get('description', 'Unknown error')}")
